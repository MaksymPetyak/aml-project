...
def BCNN_model(width=512, height=512, filename=None,
                n_classes=5, batch_size=64, p_conv=0.0, n_classes=2):
    """
    ...
    maxpool_9 = MaxPool2D(pool_size=3, strides=(2, 2))(dropout_9)   #layer 13
	
	#------------------BCNN-Specialized-part-----------------------#
	mean_pooled = GlobalAveragePooling2D( data_format = 'channels_last'
	)(maxpool_9)
	max_pooled = GlobalMaxPooling2D( data_format = 'channels_last'
	)(maxpool_9)
	global_pool = concatenate([mean_pooled,max_pooled], axis = 1)
	
	softmax_input = Dense(units = n_classes, activation = None,
	)(global_pool)
	softmax_output = Softmax()(softmax_input)
	
	model = Model(inputs = [main_input], outputs=[softmax])
